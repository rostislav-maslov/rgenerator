...
public interface GithubApi {
    ...
    public Call<List<ContentResponse>> content(@Path("name") String name, @Path("repo") String repo, @Header("Authorization") String access_token);

    @GET("/repos/{name}/{repo}/contents/{path}")
    public Call<List<ContentResponse>> contentByPath(@Path("name") String name, @Path("repo") String repo, @Path("path") String path, @Header("Authorization") String access_token);

    //Repos
    //https://api.github.com/user/repos?access_token=436cfed0a8242ba5b21f2bf70201bba06d10457a
    @GET("/user/repos")
    public Call<List<RepoResponse>> getRepos(@Header("Authorization") String access_token);
